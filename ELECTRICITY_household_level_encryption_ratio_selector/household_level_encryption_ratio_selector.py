...
if sys.platform == "win32":
    EXECUTABLE_NAME += ".exe"
GO_SOURCE_PATH = os.path.join(SCRIPT_DIR, "generate_household_level_metrics.go")
...
class EncryptionSelectorEnv(gym.Env):
    def __init__(self, dataset_type):
        ...
        df = pd.read_csv("./ELECTRICITY_household_level_encryption_ratio_selector/ML_metrics_ELECTRICITY.csv", header=0)
        df_HE = pd.read_csv("./ELECTRICITY_household_level_encryption_ratio_selector/ML_party_metrics_ELECTRICITY.csv", header=0)
...
def call_training_phase(current_leaked_plaintext_size):
    ...
    try:
        ...
        model.save(
            f"./ELECTRICITY_household_level_encryption_ratio_selector/DQN_Household_Level_Encryption_Selector_{current_leaked_plaintext_size}")
...
def call_testing_phase(current_leaked_plaintext_size):
    ...
    with open(
            f"./ELECTRICITY_household_level_encryption_ratio_selector/testing_log_{current_leaked_plaintext_size}.csv",
            "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(log_headers)
