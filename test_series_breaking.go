...
func main() {
	...
	fmt.Println("=== SERIES BREAKING ALGORITHM TEST ===")
	testSeriesBreakingAlgorithms(params)
}

func testSeriesBreakingAlgorithms(params ckks.Parameters) {
	fmt.Println("Testing series breaking algorithms for electricity data...")

	// Load real electricity dataset instead of generating synthetic data
...
func printSeriesBreakingResults(result SeriesBreakingResult) {
	...
	fmt.Printf("    Original ASR: %.4f\n", result.OriginalASR)
	fmt.Printf("    Broken ASR: %.4f\n", result.BrokenASR)
	fmt.Printf("    ASR Improvement: %.4f (%.1f%% reduction)\n",
		result.OriginalASR-result.BrokenASR,
		(result.OriginalASR-result.BrokenASR)/result.OriginalASR*100)
	fmt.Printf("    Original Time: %.2fs\n", result.OriginalTime.Seconds())
	fmt.Printf("    Broken Time: %.2fs\n", result.BrokenTime.Seconds())
	fmt.Printf("    Total Preservation: %.4f (%.1f%%)\n", result.TotalPreservation, result.TotalPreservation*100)
	fmt.Printf("    Pattern Disruption: %.4f (%.1f%%)\n", result.PatternDistruption, result.PatternDistruption*100)
	fmt.Printf("    Efficiency Gain: %.4f (%.1f%%)\n", result.EfficiencyGain, result.EfficiencyGain*100)
