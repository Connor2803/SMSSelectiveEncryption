...
def parse_block_level_log_for_fixed_encryption_ratios(log_df: pd.DataFrame, test_households: list) -> list:
    ...
    ratios = parse_ratios_string_block_level_fixed_enc(run_data.get("Per Block Encryption Ratios", ""))
    sum_errors = parse_per_party_string_block_level_fixed_enc(run_data.get("Per-Party Summation Errors", ""))
    dev_errors = parse_per_party_string_block_level_fixed_enc(run_data.get("Per-Party Deviation Errors", ""))
    enc_times = parse_per_party_string_block_level_fixed_enc(run_data.get("Per-Party Encryption Times (NS)", ""))
    dec_times = parse_per_party_string_block_level_fixed_enc(run_data.get("Per-Party Decryption Times (NS)", ""))
    sum_ops_times = parse_per_party_string_block_level_fixed_enc(run_data.get("Per-Party Summation Operations Times (NS)", ""))
    dev_ops_times = parse_per_party_string_block_level_fixed_enc(run_data.get("Per-Party Deviation Operations Times (NS)", ""))
        ...
        for target_average_ratio in target_average_ratios:
            ...
            for folder_name in folder_names:
                ...
                if "household_level_encryption_ratio_selector" in folder_name:
                    print(f"Generating metrics for {folder_name} using generate_household_level_metrics.go with leaked plaintext size:{leaked_plaintext_size}\n")
