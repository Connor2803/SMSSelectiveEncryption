...
class EncryptionSelectorEnv(gym.Env):
    def __init__(self, dataset_type: str, current_leaked_plaintext_size: str, policy_penalty: int):
        ...
        self._df = pd.read_csv("./WATER_block_level_encryption_ratio_selector_with_policy/inputs.csv", header=0)
...
def call_validation_phase(current_leaked_plaintext_size: str, current_policy_penalty: int):
    model = DQN.load(f"./WATER_block_level_encryption_ratio_selector_with_policy/DQN_Policied_Block_Level_Encryption_Ratio_Selector_{current_leaked_plaintext_size}_{current_policy_penalty}.zip")
...
def call_testing_phase(current_leaked_plaintext_size: str, current_policy_penalty: int):
    model = DQN.load(
        f"./WATER_block_level_encryption_ratio_selector_with_policy/DQN_Policied_Block_Level_Encryption_Ratio_Selector_{current_leaked_plaintext_size}_{current_policy_penalty}.zip")
    ...
    combined_test_callback = SectionLoggingCallback(
        ...
        log_path_global_test_combined=os.path.join(os.getcwd(),
                                                   f'./WATER_block_level_encryption_ratio_selector_with_policy/testing_log_{current_leaked_plaintext_size}_{current_policy_penalty}.csv'),
        verbose=0
    )
