# python ./test_model_water/test_model.py
import os
...
EXECUTABLE_NAME = "generate_test_metrics"
if sys.platform == "win32":
    EXECUTABLE_NAME += ".exe"
GO_SOURCE_PATH = os.path.join(SCRIPT_DIR, "generate_test_metrics.go")
...
class EncryptionSelectorEnv(gym.Env):
    def __init__(self, dataset_type="train"):
        ...
        df = pd.read_csv("./test_model_water/ML_metrics_WATER.csv", header=0)
        df_HE = pd.read_csv("./test_model_water/ML_party_metrics_WATER.csv", header=0)
    ...
    def step(self, action):
        ...
        info = {
            ...
            "raw_asr_attack_duration": current_asr_attack_duration,
            "raw_asr_mean": current_asr_mean,
    ...
    def _on_step(self) -> bool:
        ...
        for idx, done in enumerate(self.locals['dones']):
            if done:
                ...
                raw_asr_attack_duration = info.get("raw_asr_attack_duration", 0)
                raw_asr_mean = info.get("raw_asr_mean", 0)
                ...
                raw_encryption_time = info.get("raw_encryption_time", 0)
                raw_decryption_time = info.get("raw_decryption_time", 0)
...
def log_to_csv(writer, episode_num, household_id, reward, info):
    ...
    writer.writerow([
        ...
        info.get("raw_asr_attack_duration", 0),
        info.get("raw_asr_mean", 0),
...
def main():
    ...
    log_file_name = "./test_model_water/training_log.csv"
    csv_file = None
    csv_writer = None
    ...
    try:
        ...
        csv_writer.writerow(
            ["Episode",
             ...
             "Scaled Average Attack Duration",
             "Scaled Average ASR Mean",
        ...
        model.save("./test_model_water/DQN_Household_Level_Encryption_Selector_TEST")
    ...
    # ----- VALIDATION PHASE ------
    print("\n--- Starting Validation ---")
    model = DQN.load("./test_model_water/DQN_Household_Level_Encryption_Selector_TEST")
    env_val = EncryptionSelectorEnv(dataset_type="validation")
    env_val.reset()
    model.set_env(env_val)

    mean_reward, std_reward = evaluate_policy(model, env_val, n_eval_episodes=len(env_val._active_households), render=False)
    ...
    with open('./test_model_water/testing_log.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(log_headers)
