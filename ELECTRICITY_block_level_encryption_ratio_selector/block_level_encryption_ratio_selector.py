# python
# ./ELECTRICITY_block_level_encryption_ratio_selector/block_level_encryption_ratio_selector.py
...
class EncryptionSelectorEnv(gym.Env):
    def __init__(self, dataset_type: str):
        ...
        self._df = pd.read_csv("./ELECTRICITY_block_level_encryption_ratio_selector/inputs.csv", header=0)
    ...
    def _set_household(self, household_id):
        try:
            self._current_household_idx = self._active_households.index(household_id)
        except ValueError:
            print(f"ERROR: Household ID {household_id} not found in active households.")
            return None

        self._current_section_idx_in_household = 0

        self._current_household_data = self._df[
            self._df["Household ID"] == household_id].sort_values(by="Section Number")
...
def call_fixed_encryption_ratio_testing_phase(current_leaked_plaintext_size, target_avg_ratio):
    model_path = f"./ELECTRICITY_block_level_encryption_ratio_selector/DQN_Block_Level_Encryption_Ratio_Selector_{current_leaked_plaintext_size}.zip"
    ...
    for household_id in testing_households:
        ...
        for _, section_row in household_sections_df.iterrows():
            ...
            obs = {
                "section_level_electricity_usage": np.array([section_row["Per Section Utility Usage"]], dtype=np.float64),
    ...
    try:
        ...
        log_path = f"./ELECTRICITY_block_level_encryption_ratio_selector/testing_log_{current_leaked_plaintext_size}_{target_avg_ratio}_fixed_encryption_ratio.csv"
