# python ./test_model_electricity/test_model.py
import os
...
class EncryptionSelectorEnv(gym.Env):
    def __init__(self, dataset_type="train"):
        ...
        df = pd.read_csv("./test_model_electricity/ML_metrics_ELECTRICITY.csv", header=0)
        df_HE = pd.read_csv("./test_model_electricity/ML_party_metrics_ELECTRICITY.csv", header=0)
...
def main():
    ...
    log_file_name = f"./test_model_electricity/training_log_{currentAtdSize}.csv"
    csv_file = None
    csv_writer = None
    ...
    try:
        ...
        model.save("./test_model_electricity/DQN_Household_Level_Encryption_Selector_TEST")
    ...
    # ----- VALIDATION PHASE ------
    print("\n--- Starting Validation ---")
    model = DQN.load("./test_model_electricity/DQN_Household_Level_Encryption_Selector_TEST")
    ...
    with open(f"./test_model_electricity/testing_log_{currentAtdSize}.csv", "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(log_headers)
