# python ./ELECTRICITY_block_level_encryption_ratio_selector_with_policy/block_level_encryption_ratio_selector_with_policy.py 12
...
EXECUTABLE_NAME = "generate_block_level_policy_metrics"
if sys.platform == "win32":
    EXECUTABLE_NAME += ".exe"
GO_SOURCE_PATH = os.path.join(SCRIPT_DIR, "generate_block_level_policy_metrics.go")
...
class Welford:
    ...
    def update(self, current_reidentification_rate, current_advanced_reidentification_rate,
        ...
        old_avg_encryption_ratio_M = self.avg_encryption_ratio_M
        ...
        self.avg_encryption_ratio_M += (current_avg_encryption_ratio - self.avg_encryption_ratio_M) / self.k
        ...
        self.avg_encryption_ratio_S += (current_avg_encryption_ratio - self.avg_encryption_ratio_M) * (current_avg_encryption_ratio - old_avg_encryption_ratio_M)
    ...
    def get_variance(self):
        ...
        self.avg_encryption_ratio_V = self.avg_encryption_ratio_S / (self.k - 1)
    ...
    def get_standardised_values(self, current_reidentification_rate, current_advanced_reidentification_rate,
        ...
        self.deviation_operations_time_Z = (current_deviation_operations_time - self.deviation_operations_time_M) / \
                                           std_devs[9]
        self.avg_encryption_ratio_Z = (current_avg_encryption_ratio - self.avg_encryption_ratio_M) / std_devs[10]
...
class EncryptionSelectorEnv(gym.Env):
    def __init__(self, dataset_type: str):
        ...
        self._df = pd.read_csv("./ELECTRICITY_block_level_encryption_ratio_selector_with_policy/inputs.csv", header=0)
    ...
    def step(self, action):
        ...
        if terminated:
            ...
            try:
                ...
                data_for_go_filepath = os.path.join(SCRIPT_DIR, "RL_V2_5_choices.json")
                ...
                total_ratio = 0
                for choice in self._chosen_encryption_ratios.values():
                    total_ratio += choice["ratio"]

                avg_encryption_ratio = total_ratio / len(self._chosen_encryption_ratios)
                ...
                z_reidentification_rate, z_advanced_reidentification_rate, z_reidentification_duration, z_memory, z_sum_error, z_dev_error, z_enc_time, z_dec_time, z_sum_ops_time, z_dev_ops_time, z_avg_encryption_ratio = z_scores
                ...
                if global_advanced_reidentification_rate > target_reid_rate:
                    reid_penalty += (global_advanced_reidentification_rate - target_reid_rate) * penalty_multiplier

                # 4. Calculate the final reward.
                reward = intermediate_reward - privacy_cost - utility_cost - reid_penalty
...
def main():
    ...
    logging_callback = SectionLoggingCallback(
        ...
        log_path_global_train=os.path.join(os.getcwd(), f'./ELECTRICITY_block_level_encryption_ratio_selector_with_policy/training_log_{currentLeakedPlaintextSize}.csv'),
    ...
    model.save(f"./ELECTRICITY_block_level_encryption_ratio_selector_with_policy/DQN_Policied_Block_Level_Encryption_Ratio_Selector_{currentLeakedPlaintextSize}")
    del model

    # ----- VALIDATION PHASE ------
    print("\n----- VALIDATION PHASE BEGIN ------")
    model = DQN.load(f"./ELECTRICITY_block_level_encryption_ratio_selector_with_policy/DQN_Policied_Block_Level_Encryption_Ratio_Selector_{currentLeakedPlaintextSize}")
    ...
    combined_test_callback = SectionLoggingCallback(
        ...
        log_path_global_test_combined=os.path.join(os.getcwd(),
                                                   f'./ELECTRICITY_block_level_encryption_ratio_selector_with_policy/testing_log_{currentLeakedPlaintextSize}.csv'),
        verbose=0
    )
    ...
    #
    # model = DQN.load("DQN_Encryption_Ratio_Selector_V2_5")
    #
    ...
    #     log_path_global_train=None,
    #     log_path_global_test_ph=os.path.join(os.getcwd(), 'V2_5_testing_log_ph.csv'),
    #     log_path_global_test_combined=None,
    #     verbose=0
    # )
    #
